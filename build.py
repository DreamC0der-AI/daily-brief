...
T = {  # UI strings
    "en": dict(archive="Archive", share="Share", indepth="In depth", briefs="Briefs", items="items", curated="curated", auto="auto",
               ...
               site_title="Daily Brief", desc_suffix="", wx_guide_title="Share from WeChat's own menu", wx_guide_text="Tap the ··· button in the top-right corner and choose Send to Chat. WeChat builds the card from this page.", wx_guide_tap="Tap anywhere to close", moments="Moments", moments_title="Share to Moments", moments_text="Tap the ··· button in the top-right corner and choose Share to Moments. WeChat builds the card from this page; tapping the card opens it.", moments_qr="Scan with WeChat to open this page there, then tap ··· in the top-right and choose Share to Moments. The link is also copied."),
    "zh": dict(archive="往期", share="分享", indepth="深度", briefs="简讯", items="条", curated="AI 精选", auto="自动",
               readat="阅读原文（{src}）→", share_page="分享今日简报", share_item="分享这条新闻", wechat="微信", wechat_hint="用微信扫一扫，或复制链接后粘贴到聊天中",
               copy="复制链接", copied="已复制", copytext="复制文字", native="更多…", other_lang="EN", intro_auto="自动挑选的热门条目，本日尚未由 AI 整理。",
               site_title="Daily Brief 每日简报", desc_suffix="", wx_guide_title="请用微信右上角菜单分享", wx_guide_text="点击右上角的 ··· 按钮，选择“发送给朋友”，微信会自动生成本页的卡片。", wx_guide_tap="点击任意位置关闭", moments="朋友圈", moments_title="分享到朋友圈", moments_text="点击右上角的 ··· 按钮，选择“分享到朋友圈”。微信会自动生成本页卡片，点击卡片即可打开。", moments_qr="用微信扫码打开本页，再点右上角 ··· 选择“分享到朋友圈”。链接也已复制。"),
}
...
def render_page(d, dates, curated, lang):
    ...
    repl = {
        ...
        "{{PAGE_QR}}": og.qr_svg(page_url), "{{SQ_IMAGE}}": sq_img, "{{WX_GUIDE_TITLE}}": s["wx_guide_title"], "{{WX_GUIDE_TEXT}}": s["wx_guide_text"], "{{WX_GUIDE_TAP}}": s["wx_guide_tap"], "{{MOMENTS}}": s["moments"], "{{MOMENTS_TITLE}}": s["moments_title"], "{{MOMENTS_TEXT}}": s["moments_text"], "{{MOMENTS_QR}}": s["moments_qr"], "{{ICON}}": ICON_SHARE, "{{INDEX_HREF}}": "index.html" if lang == "en" else "index.zh.html",
    }
    out = TEMPLATE
